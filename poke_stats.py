...
class PokeStatsStore:
    ...
    def _empty_data() -> dict[str, Any]:
        return {"groups": {}, "meta": {"updated_at": None}}
    ...
    def _load(self) -> dict[str, Any]:
        ...
        if not isinstance(meta, dict):
            meta = {}
        return {"groups": groups, "meta": {"updated_at": meta.get("updated_at")}}
    ...
    def record(self, group_id: str, user_id: str) -> int:
        ...
        self.data.setdefault("meta", {})["updated_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self._save()
        return user["count"]

    def group_summary(self, group_id: str, target_user_id: str | None = None) -> dict[str, Any]:
        group = self.data.get("groups", {}).get(str(group_id), {})
        users = group.get("users", {}) if isinstance(group, dict) else {}
        entries = sorted(
            ((str(uid), int(info.get("count", 0))) for uid, info in users.items() if isinstance(info, dict)),
            key=lambda item: (-item[1], item[0]),
        )
        target_count = None
        if target_user_id is not None:
            info = users.get(str(target_user_id), {}) if isinstance(users, dict) else {}
        ...
        return {
            ...
            "unique_users": len(users) if isinstance(users, dict) else 0,
